...
class DumpParser:
    ...
    def __init__(self, dump_path: str):
        ...
        self._constants: GameConstantsInfo | None = None
    ...
    def _map_skill_category_to_type(self, cat_name: str) -> str:
        ...
        if "bow" in cat_lower:
            return "bow"
        if "bomb" in cat_lower:
            return "bomb"
        if "gun" in cat_lower:
            return "gun"
        if "melee" in cat_lower:
            return "melee"
        if "machinegun" in cat_lower:
            return "machine_gun"
        if "spear" in cat_lower:
            return "spear"
        if "boomerang" in cat_lower:
            return "boomerang"
        if "medkit" in cat_lower:
            return "medkit"
        if "consumable" in cat_lower:
            return "consumable"
        if "passive" in cat_lower:
            return "passive"
        if "arrowrain" in cat_lower:
            return "arrow_rain"
        if "spartan" in cat_lower:
            return "spartan"
        if "focus" in cat_lower:
            return "focus"
    ...
    def parse_constants(self) -> GameConstantsInfo:
        ...
        print("[DumpParser] Parsed game constants")
    ...
    def export_to_json(self, output_dir: str | Path) -> None:
        ...
        # Characters
        characters_data = {char_id: char.to_dict() for char_id, char in self._characters.items()}
        with open(output_dir / "characters.json", "w") as f:
            json.dump(characters_data, f, indent=2)

        # Skills
        skills_data = {skill_id: skill.to_dict() for skill_id, skill in self._skills.items()}
        with open(output_dir / "skills.json", "w") as f:
            json.dump(skills_data, f, indent=2)

        # Weapons
        weapons_data = {weapon_id: weapon.to_dict() for weapon_id, weapon in self._weapons.items()}
        with open(output_dir / "weapons.json", "w") as f:
            json.dump(weapons_data, f, indent=2)

        # Projectiles
        projectiles_data = {proj_id: proj.to_dict() for proj_id, proj in self._projectiles.items()}
        with open(output_dir / "projectiles.json", "w") as f:
            json.dump(projectiles_data, f, indent=2)

        # NPCs
        npcs_data = {npc_id: npc.to_dict() for npc_id, npc in self._npcs.items()}
        with open(output_dir / "npcs.json", "w") as f:
            json.dump(npcs_data, f, indent=2)

        # Buffs
        buffs_data = {buff_id: buff.to_dict() for buff_id, buff in self._buffs.items()}
    ...
    def get_character_by_name(self, name: str) -> CharacterInfo | None:
        """Get character by name."""
        char_id = self._name_to_id(name).lower()
        return self._characters.get(char_id)

    def get_character_by_enum_id(self, enum_id: int) -> CharacterInfo | None:
